...
i = int(sys.argv[1]) #combined with arrjoy to run several simulations at the same time
...
def simulate_cicle(growth_steps):
    for simulation_time in growth_steps:
        ...
        np.save(T+"/times_"+extension,times)
        np.save(T+"/senders_"+extension,senders)
        nest.SetStatus(spike_detector,'n_events',0)

        del local_connections

        np.save(T+"/sources_"+extension,sources)
        np.save(T+"/targets_"+extension,targets)
        np.save(T+"/weights_"+extension,weights)
...
def simulate_cycle2(growth_steps):
    #This simulation cycle has shorter interval 
    for simulation_time in growth_steps:
        ...
        np.save(T+"/DC_times_"+extension,times)
        np.save(T+"/DC_senders_"+extension,senders)
        nest.SetStatus(spike_detector,'n_events',0)
        del local_connections
        np.save(T+"/DC_sources_"+extension,sources)
        np.save(T+"/DC_targets_"+extension,targets)
        np.save(T+"/DC_weights_"+extension,weights)


angles = [0.]*N
projections = [{"I_e":np.cos(angles[x-1])*elec_field_stren} for x in pop_exc[0:N]] #define DC injection intensity
nest.SetStatus(pop_exc[0:N],projections)
simulate_cycle2(growth_steps_DC_on)


growth_steps_DC_off = np.arange(growth_time+2*growth_step, growth_time+6*growth_step,stimulate_step)
def simulate_cycle3(growth_steps):
    for simulation_time in growth_steps:
        nest.SetStatus(spike_detector,{"start": simulation_time+stimulate_step-5000.,"stop": simulation_time+stimulate_step})
        nest.Simulate(stimulate_step)
        local_connections = nest.GetConnections(pop_exc, pop_exc)
        ...
        senders = events['senders']
        extension = str(simulation_time+stimulate_step)+'_seed_'+str(i)+".npy"
        np.save(T+"/DC_off_times_"+extension,times)
        np.save(T+"/DC_off_senders_"+extension,senders)
        nest.SetStatus(spike_detector,'n_events',0)
        del local_connections

        np.save(T+"/DC_off_sources_"+extension,sources)
        np.save(T+"/DC_off_targets_"+extension,targets)
        np.save(T+"/DC_off_weights_"+extension,weights)



projections = [{"I_e":0.} for x in pop_exc]
nest.SetStatus(pop_exc,projections)
simulate_cycle3(growth_steps_DC_off)
